...
# 文件头描述格式化文本
lua_file_head_format_desc = '''--[[ %s from file:%s    --]]\n'''
...
def excel2lua(src_excel_path, tgt_lua_path):
    ...
    for k, v in excel_data_dict.items():
        lua_export_file.write('  [%d]={' % k)#        lua_export_file.write('  [%d] = {\n' % k)
        for row_data in v:
            lua_export_file.write('{0}={1},'.format(row_data[0], row_data[1]))#            lua_export_file.write('   {0} = {1},'.format(row_data[0], row_data[1]))
...
if __name__ == '__main__':
    # 使用bat导入整个目录
    excPath=os.path.join(curpath,sys.argv[1])
    luaPath=os.path.join(curpath,sys.argv[2])
    for root, dirs, files in os.walk(excPath):
        for file in files :
            print("开始导表="+file)
            exc=os.path.join(excPath,file)
            lua = os.path.join(luaPath, file.replace(".xlsx",".lua"))
            excel2lua(exc,lua)

    # 使用bat导入
    # excel2lua(os.path.join(curpath, sys.argv[1]), os.path.join(curpath, sys.argv[2]))#--单独一个的

    # 使用当前文件夹内的测试
    # excel2lua("data.xlsx", "hello.lua")--测试
